...
# Automate changelog generation in the continuous integration pipeline
...
### 1. Store the API key as a repository secret
...
:::caution `fetch-depth: 0` isn't optional
...
The step preceding writes to `CHANGELOG.md` but doesn't persist it. Add a final step to commit
the change back:
...
- [CLI reference](../reference/cli-reference.md) for every flag used.
